    ...
    const FOBB2D& Src, const FOBB2D& Dest)
{
    ...
    if (!ComputeAxisProjection(CenterLine,
        Src.Axis[(int)EAxis::X], Src.HalfSize.x,
        Dest.Axis, Dest.HalfSize))
        return false;

    if (!ComputeAxisProjection(CenterLine,
        Src.Axis[(int)EAxis::Y], Src.HalfSize.y,
        Dest.Axis, Dest.HalfSize))
        return false;

    if (!ComputeAxisProjection(CenterLine,
        Dest.Axis[(int)EAxis::X], Dest.HalfSize.x,
        Src.Axis, Src.HalfSize))
        return false;

    if (!ComputeAxisProjection(CenterLine,
        Dest.Axis[(int)EAxis::Y], Dest.HalfSize.y,
        Src.Axis, Src.HalfSize))
        return false;
    ...
    float SrcRadius, const FOBB2D& Dest)
{
    ...
    float CenterProjectionDist = 
        abs(CenterLine.Dot(Dest.Axis[(int)EAxis::X]));
    ...
    CenterProjectionDist = 
        abs(CenterLine.Dot(Dest.Axis[(int)EAxis::Y]));
    ...
    const FVector2D& DestHalfSize)
{
    ...
    float DestProjectionDist =
        abs(SeparationAxis.Dot(DestAxis[(int)EAxis::X])) * DestHalfSize.x +
        abs(SeparationAxis.Dot(DestAxis[(int)EAxis::Y])) * DestHalfSize.y;

    if (CenterProjectionDist > AxisHalfSize + DestProjectionDist)
        return false;
...
FOBB2D CCollision::CreateOBB2D(const FAABB2D& Info)
{
    ...
    result.Axis[(int)EAxis::X].x = 1.f;
    result.Axis[(int)EAxis::X].y = 0.f;

    result.Axis[(int)EAxis::Y].x = 0.f;
    result.Axis[(int)EAxis::Y].y = 1.f;
